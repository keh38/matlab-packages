function unparsed = parse_propval_pairs(varargin)
% parse_propval_pairs -- parse input argument list.
% Usage parse_propval_pairs('prop1', val1, 'prop2', val2, ...)
...
while k <= nargin
   if ischar(varargin{k}) && any(strcmpi(varargin{k}, callerVars)) && k<nargin
      icaller = strcmpi(varargin{k}, callerVars);
      assignin('caller', callerVars{icaller}, varargin{k+1});
      k = k + 2;
   else
      unparsed{end+1} = varargin{k}; %#ok<AGROW> 
      k = k + 1;
   end
end
