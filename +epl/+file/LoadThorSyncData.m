...
clockRate = 20000000;
sampleRate = epl.file.LoadSyncXML(fileparts(filepath));
...
if ~isempty(varargin)
   assert(rem(length(varargin),2)==0 && iscellstr(varargin(1:2:end)), 'Inputs failed to conform to expected string-value pair format, eg> ''start'',1'); %#ok<ISCLSTR>
