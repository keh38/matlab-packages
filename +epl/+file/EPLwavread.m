...
if ~isempty(ext) && numel(ext)==1 && ext==-1,
...
nread = diff(ext) + 1;
startPoint = mod(ext(1)-1, SamplesPerChannel) + 1;
endPoint = startPoint + nread - 1;

bytes_remaining = total_bytes;  % Preset byte counter
skipcnt = BytesPerSample * (startPoint-1) * wavefmt.nChannels;

if endPoint > SamplesPerChannel,
   nptsFromBeginning = min(endPoint - SamplesPerChannel, startPoint-1);
   endPoint = SamplesPerChannel;
   tail = fread(datack.fid, [wavefmt.nChannels nptsFromBeginning], dtype);
   
   tailBytes = BytesPerSample * numel(tail);
   skipcnt = skipcnt - tailBytes;
   bytes_remaining = bytes_remaining - tailBytes;
else
   tail = [];
end
