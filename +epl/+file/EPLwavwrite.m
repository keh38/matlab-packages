...
if ~isstruct(y),
   ...
   fbin = fopen(y.filename, 'rb');
   
   numLeftToRead = samples;
   nread = 100000;
   
   while numLeftToRead > 0,
      nread = min(nread, numLeftToRead);
      x = fread(fbin, nread, 'double');
      x = x / y.maxAbsVal;
      error(write_wavedat(fid,fmt,x));
      numLeftToRead = numLeftToRead - nread;
   end   
...
for k = 1:2:nprop,
   ...
   if rem(length(names{end}), 2), names{end} = [names{end} ' ']; end
