...
% added by keh 8/20/14: option to force length 1 cells to be treated as
% arrays. Otherwise, json.net is not happy deserializing lists
% (collections) in C#.
forceCellArray = jsonopt('ForceCellArray',0,varargin{:});

dim=size(item);
len=numel(item); % let's handle 1D cell first
padding1=repmat(sprintf('\t'),1,level-1);
padding0=repmat(sprintf('\t'),1,level);
if(len>1 || forceCellArray), 
    if(~isempty(name))
...
if(len>1 || forceCellArray), txt=sprintf('%s\n%s]',txt,padding0); end
