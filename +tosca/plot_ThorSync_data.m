function varargout = plot_ThorSync_data(folder, varargin)

fn = fullfile(folder, 'tsd.mat');

if exist(fn, 'file')
   load(fn, 'tframe', 'ttrial', 'trep', 'tstate');
else
   fn = fullfile(folder, 'Episode001.h5');
   s = epl.file.LoadThorSyncData(fn, ...
      'vars', {'time', 'FrameOut', 'ToscaRep', 'ToscaState', 'ToscaTrial'}, ...
      varargin{:});

   tframe = s.time(diff(s.FrameOut) > 0);
   trep = s.time(diff(s.ToscaRep) > 0);
   tstate = s.time(diff(s.ToscaState) > 0);
   ttrial = s.time(diff(s.ToscaTrial) > 0);
