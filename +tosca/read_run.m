function [Data, Params] = read_run(fn, check)
...
if check && ~isempty(Data) && (~isfield(Params.Info, 'AlignmentChecked') || (isnumeric(Params.Info.Version) && Params.Info.Version < 1988))
   Data = check_alignment(Params, Data);
end
...
while 1
   ...
   curKey = epl.file.create_valid_varname(s(1:idx-1));
...
%--------------------------------------------------------------------------
function P = cleanup_params(P)
F = [];
for k = 1:length(P.Tosca.Schedule.Families)
   f = P.Tosca.Schedule.Families(k).Family;
   V = [];
   for kv = 1:length(f.Vars)
      V = [V; f.Vars(kv).Element];
   end
   f.Vars = V;
   F = [F; f];
end

P.Tosca.Schedule.Families = F;
%--------------------------------------------------------------------------
