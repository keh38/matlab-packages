...
numTraceTrialEvents = 0;
ktr = itrace;
while true
   if tr(ktr).Time(1) < max(s.Time_s)
      numTraceTrialEvents = numTraceTrialEvents + 1;
   else
      break;
   end
...
if isempty(tRep)
   tState = di_time(diff([0 s.State_Change]) > 0.5);
   tStateEnqueued = tr.Time(tr.Event==2) - s.Time_s(1);
   for k = 1:length(tStateEnqueued)
      dt = min(abs(tState - tStateEnqueued(k)));
      if dt > 0.5
         tInsert = tStateEnqueued(k) + 2*s.frameSize;
         tInsert = ceil(tInsert/s.frameSize) * s.frameSize;
         
         insert_at = find(di_time < tInsert, 1, 'last');
         if isempty(insert_at)
            return;
         end
   ...
   for k = 1:length(idxWrite1)
      ...
      if ~any(tRep >= tRepMarker - 5e-3 & tRep < tRepMarker + 20e-3)
         ...
         if insert_at == length(di_time)
            ...
            tState = s.Time_s(diff(s.State_Change)>0.25);
            ...
            tReps = s.Time_s(diff(s.Rep_Trigger) < -0.25) - s.Time_s(1);
...
if any(tRep > tEnd) || numTraceTrialEvents > 2
