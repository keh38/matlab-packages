function [TL, avi] = repair_video(TL, avi)
...
for k = nmax:-1:aviErrorTrial+1
   movefile( ...
      fullfile(folder, sprintf('%s.%03d.avi', filestem, k)), ...
      fullfile(folder, sprintf('%s.%03d.avi', filestem, k+1)));
end

%% Split the video
fnPre = fullfile(folder, sprintf('%s.%03d.avi', filestem, aviErrorTrial));
fnPost = fullfile(folder, sprintf('%s.%03d.avi', filestem, aviErrorTrial+1));
...
vidPre.Quality = 80;
vidPre.FrameRate = vidPrebig.FrameRate;
...
vidPost.Quality = 80;
vidPost.FrameRate = vidPrebig.FrameRate;
