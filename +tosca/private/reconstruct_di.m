function Data = reconstruct_di(P, Data)
...
Trace = tosca_read_trace_data(P.Info.Filename);
flatTrace = tosca_read_trace_data(P.Info.Filename, true);
...
%       idi_out = idi_out + 1;
   ...
   if s.Rep_Trigger(1) > 0.5
      tRep = [s.Time_s(1) tRep]; 
   end
%    missingTrialMarker = any(tRep > tEnd);
   ...
   numTraceTrialEvents = find_trials_in_trace(s, flatTrace, itrace);
   if numTraceTrialEvents ~= 2
%       fprintf('NumTraceTrialEvents = %d\n', numTraceTrialEvents);
   end
   if numTraceTrialEvents <=2 && numStateChange == length(s.History) / 2
...
%--------------------------------------------------------------------------
function N = find_trials_in_trace(S, TR, trialNum)

itrial = find(TR.Event == 1);
i1 = itrial(trialNum);

N = sum(TR.Event(i1:end) == 1 & TR.Time(i1:end) < max(S.Time_s));
