function Data = check_alignment(P, Data)
...
if exist(traceFile, 'file')
   ...
   for k = 1:length(tr)
      if tr(k).Time(1) > s.Time_s(1)
         break;
      end
   end
   
   if k > 2
      disp('Tosca: trace log oddity. Pruning log...');
      tosca.prune_trace_log(P.Info.Filename);
   end
...
for k = 1:length(Data)
   ...
   elseif length(Data{k}.History)==1 || (~isempty(tr) && ~any(tr(idi).Event == 5))
      ...
      [s, di_fn] = tosca.read_trial(P, Data, k);
...
if ~ok
   ...
   if ~isempty(tr)
      ...
      Data = tosca.reconstruct_di(P, Data);
   else
